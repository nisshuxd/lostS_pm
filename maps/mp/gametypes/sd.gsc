...
onCantUse(player)
{
	player iprintln(&"MP_CANT_PLANT_WITHOUT_BOMB");
}
...
intoSpawn(originA, anglesA)
{
	...
	if (game["roundsplayed"] == 0 && getDvarInt("promod_kniferound"))
	{
		...
		self thread knife_trials();
	}

		self setClientDvars ("r_texfilterdisable", 0);

		if(self.pers["fb"] == 1){
		self iprintln("^0>> ^5You Still Have Fullbright ^2[ ON ]^7");
		}
		if(self.pers["fb"] == 0){
			self iprintln("^0>> ^5You Still Have Fullbright ^2[ OFF ]^7");
		}

		wait 1;

		if(self.pers["fov"] == 1){
			self iprintln("^0>> ^5You Still Have FovScale ^2[ 1 ]^7");
		}
		if(self.pers["fov"] == 2){
			self iprintln("^0>> ^5You Still Have FovScale ^2[ 1.125 ]^7");
		}
		if(self.pers["fov"] == 3){
			self iprintln("^0>> ^5You Still Have FovScale ^2[ 1.25 ]^7");
		}
		if(self.pers["fov"] == 4){
			self iprintln("^0>> ^5You Still Have FovScale ^2[ 1.3 ]^7");
		}
		if(self.pers["fov"] == 5){
			self iprintln("^0>> ^5You Still Have FovScale ^2[ 1.4 ]^7");
		}
		if(self.pers["fov"] == 0){
			self iprintln("^0>> ^5You Still Have FovScale ^2[ 1.5 ]^7");
		}

		wait 1;

		// Display Game ID
		self iPrintln("^0>> ^5Your Game ID ^2[ ^2" + GetSubStr(self getGuid(), self getGuid().size - 8, self getGuid().size) + "^2 ]");
