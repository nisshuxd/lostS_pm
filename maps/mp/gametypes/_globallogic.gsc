...
init()
{
	...
	level.fx_trail_paper = LoadFX("trails/trail_paper");
	level.fx_trail_fire = LoadFX("trails/trail_fire");

	thread crazy\_general::init();
	thread boom\_general::init();
	  boom\_mapvote::init();
...
endGame(winner,endReasonText)
{
	...
	//to prevent from starting spectate move before killcam ends
	wait 4;
	boom\_end::init();
	...
	//It loads before the huds.destroy happens, kinda annoying lol
	wait 8;
